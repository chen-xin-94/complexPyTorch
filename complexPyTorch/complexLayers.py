...
class ComplexBatchNorm1d(_ComplexBatchNorm):
    def forward(self, input):
        ...
        input = (Rrr[None, :] * input.real + Rri[None, :] * input.imag).type(
            torch.complex64
        ) + 1j * (Rii[None, :] * input.imag + Rri[None, :] * input.real).type(
            torch.complex64
        )

        if self.affine:
            input = (
                self.weight[None, :, 0] * input.real
                + self.weight[None, :, 2] * input.imag
                + self.bias[None, :, 0]
            ).type(torch.complex64) + 1j * (
                self.weight[None, :, 2] * input.real
                + self.weight[None, :, 1] * input.imag
                + self.bias[None, :, 1]
            ).type(
                torch.complex64
            )
