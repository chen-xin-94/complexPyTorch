...
def complex_sigmoid(input):
    return torch.sigmoid(input.real).type(torch.complex64) + 1j * torch.sigmoid(
        input.imag
...
def complex_opposite(input):
    return -(input.real).type(torch.complex64) + 1j * (
        -(input.imag).type(torch.complex64)
    )
...
def complex_stack(input, dim):
    ...
    return torch.stack(input_real, dim).type(torch.complex64) + 1j * torch.stack(
        input_imag, dim
    ).type(torch.complex64)
